            ...
            @if (!in_array($inventory->warehouse_id, $warehouseIds))
                ...
                <ul class="shopping-cart-summary">
                    ...
                    <li>
                        ...
                        <span>{{ $inventory->warehouse->opening_time }} -
                            {{ $inventory->warehouse->close_time }}</span>
                    </li>
                    <li>
...
<div class="form-group mb-4">
    <div class="checkbox">
        <label>
            {!! Form::checkbox('agree', null, null, ['class' => 'i-check-blue', 'required']) !!} {!! trans('theme.input_label.i_agree_with_terms', [
                'url' => route('page.open', \App\Models\Page::PAGE_TNC_FOR_CUSTOMER),
            ]) !!}
        </label>
    </div>
...
<div id="submit-btn-block" class="clearfix space30" style="display: none;">
    ...
    <a href="javascript:void(0)" id="paypal-express-btn" class="hide" type="submit">
        <img src="{{ asset(sys_image_path('payment-methods') . 'paypal-express.png') }}" width="70%"
            alt="paypal express checkout" title="paypal-express" />
    </a>
</div>
