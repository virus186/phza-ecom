...
@push('script')
  <script>
    ...
    const copy_to_clipboard = (object) => {
      ...
      switch (id) {
        case 'copy-key':
          data = '{{ env('PHZA24_API_KEY') ?: env('PHZA24_API_KEY') }}';
          $(object).html(' {{ trans('app.copied') }}')
          break;
        case 'copy-secret':
          $(object).html(' {{ trans('app.copied') }}')
          data = '{{ env('PHZA24_ENCRYPTION_KEY') ?: env('PHZA24_ENCRYPTION_KEY') }}';
          break;
        case 'copy-iv':
          $(object).html(' {{ trans('app.copied') }}')
          data = '{{ env('PHZA24_ENCRYPTION_IV') ?: env('PHZA24_ENCRYPTION_IV') }}';
          break;
        default:
          break;
      }
