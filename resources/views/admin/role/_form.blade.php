  ...
  <table class="table table-striped" id="tbl-permissions">
    <tbody>
      ...
      @foreach ($modules as $module)
        ...
        @endphp
        <tr class="{{ $access_level . '-module' }}" {{ 'common' == $access_level || ('merchant' == $access_level && $merchant_user) || (isset($role) && (($role->public == 1 && 'merchant' == $access_level) || ($role->id == config('installation.seed.merchant_role_id') && 'merchant' == $access_level) || ($role->public == 0 && 'platform' == $access_level && $role->id != config('installation.seed.merchant_role_id')))) ? 'show' : 'hidden' }}>
          <td>
