...
class CartController extends Controller
{
    ...
    public function validateCoupon(Request $request, Cart $cart)
    {
        ...
        if (!$coupon) {
            return response()->json([
                'message' => trans('coupons::lang.coupon_not_exist')
            ], 404);
        }

        if (!$coupon->isLive() || !$coupon->isValidCustomer(Auth::guard('api')->id())) {
            return response()->json([
                'message' => trans('coupons::lang.coupon_not_valid')
            ], 412);
        }
        ...
        if (!$coupon->isValidZone($request->zone)) {
            return response()->json([
                'message' => trans('coupons::lang.coupon_not_valid_for_zone')
            ], 412);
        }

        if (!$coupon->hasQtt()) {
            return response()->json([
                'message' => trans('coupons::lang.coupon_limit_expired')
            ], 412);
        }
        ...
        return response()->json([
            'message' => trans('coupons::lang.coupon_applied'),
            'cart' => new CartResource($cart),
        ], 200);
    }
