              ...
              <div class="image-banner {{ $banner['columns'] > 11 ? 'single-banner' : '' }}">
                <div class="shell-banner__box {{ !empty($banner['effect']) ? 'outline-effect' : '' }}">
                  <div class="shell-banner__img">
                    <img src="{{ isset($banner['feature_image']['path']) && Storage::exists($banner['feature_image']['path']) ? get_storage_file_url($banner['feature_image']['path'], 'full') : get_storage_file_url() }}" alt="{{ $banner['title'] ?? 'Banner Image' }}" title="{{ $banner['title'] ?? 'Banner Image' }}">
                  </div>
